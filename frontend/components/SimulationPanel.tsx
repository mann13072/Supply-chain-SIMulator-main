...
const Toggle = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
  <button
    ...
    className={`relative shrink-0 w-11 h-6 rounded-full transition-all duration-300 focus:outline-none ${
      checked ? 'bg-white shadow-[0_0_12px_rgba(255,255,255,0.3)]' : 'bg-white/10'
    }`}
  >
    <span className={`absolute top-1 w-4 h-4 rounded-full transition-transform duration-300 shadow-sm ${
